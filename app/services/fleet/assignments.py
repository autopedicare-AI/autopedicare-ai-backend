...
class AssignmentService:
    ...
    def create_assignment(self, assignment_data: AssignmentCreate) -> AssignmentResponse:
        ...
        active_driver = self.db.query(Assignment).filter(
            Assignment.driver_id == assignment_data.driver_id,
            Assignment.status == "active"
        ).first()
        if active_driver:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Driver already assigned to a vehicle")

        active_vehicle = self.db.query(Assignment).filter(
            Assignment.vehicle_id == assignment_data.vehicle_id,
            Assignment.status == "active"
        ).first()
        if active_vehicle:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Vehicle already assigned to a driver")
    ...
    def update_assignment(self, assignment_id: str, assignment_data: AssignmentUpdate) -> AssignmentResponse:
        try:
        ...
        assignment = self.db.query(Assignment).filter(Assignment.id == assignment_uuid).first()
        if not assignment:
